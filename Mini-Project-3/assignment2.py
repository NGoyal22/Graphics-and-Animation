...
reader.SetFileName("Cute_little_putinpusV11.stl")
reader.Update()
polydata = reader.GetOutputPort()
number_of_vertices = reader.GetOutput().GetNumberOfPoints()

#Step 2: Create a plane using vtkPlane
center = reader.GetOutput().GetCenter()
plane = vtk.vtkPlane()
plane.SetOrigin(center)
plane.SetNormal(1, 0, 1) 

#Step 3: Clip the data using vtkClipPolyData 
...
#Step 4: Intersection area between the plane and polygonal data
...
a = mesh.Mesh.from_file('Cute_little_putinpusV11.stl')
points = np.around(np.unique(a.vectors.reshape([int(a.vectors.size/3),3]), axis=0), 2)
print("Number of vertices in Original Model: ", len(points))
